...
def _fetch_families(api_key):
    ...
    url = f"https://www.googleapis.com/webfonts/v1/webfonts?sort=ALPHA&key={api_key}"
    try:
        ...
        _log.warning(
            "plone.palette: Google Fonts API fetch failed, using fallback list"
        )
...
def _get_api_key():
    try:
        return (
            api.portal.get_registry_record("plone.palette.google_fonts_api_key") or ""
        )
    except Exception:
        return ""
